...
**Repeat visits:** If the user opens the same link again within a few minutes, the redirect can use cached data so it’s faster (no extra backend call).
...
2. **Resolve the provider**  
   We get the application’s list of providers (from the backend or from a short-lived cache). We find the one whose name or id matches the parameter.
...
4. **Do this as early as possible**  
   We run this logic as soon as we have the application data (right after the API responds, or from cache). If we redirect, we never draw the provider list.

5. **Optional cache**  
   After a successful load, we store the application data in the browser for a short time. If the user hits the same kind of link again, we can redirect without calling the backend again.
...
| File | What changed |
|------|----------------|
| `web/src/auth/LoginPage.js` | Added logic to read `id`, find the provider, and redirect (OAuth or SAML) before showing the list; optional cache for faster repeat visits. |
