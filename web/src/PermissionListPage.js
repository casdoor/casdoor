...
class PermissionListPage extends BaseListPage {
  ...
  renderTable(permissions) {
    ...
    return (
      <div>
        <Table scroll={{x: "max-content", y: 619}} columns={columns} dataSource={permissions} rowKey={(record) => `${record.owner}/${record.name}`} size="middle" bordered pagination={paginationProps}
          title={() => (
            <div>
