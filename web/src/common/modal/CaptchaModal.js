...
export const CaptchaModal = (props) => {
  const {owner, name, visible, onOk, onUpdateToken, onCancel, isCurrentProvider, noModal, innerRef, organization, username} = props;
  ...
  const loadCaptcha = () => {
    UserBackend.getCaptcha(owner, name, isCurrentProvider, organization, username).then((res) => {
      if (res.type === "none") {
        handleOk();
