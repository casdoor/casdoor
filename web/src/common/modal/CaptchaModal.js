...
export const CaptchaModal = (props) => {
  const {owner, name, visible, onOk, onUpdateToken, onCancel, isCurrentProvider, noModal, innerRef, dest} = props;
  ...
  const loadCaptcha = () => {
    UserBackend.getCaptcha(owner, name, isCurrentProvider, dest).then((res) => {
      if (res.type === "none") {
        handleOk();
