  ...
  renderApplication() {
    return (
      <Card size="small" title={
        ...
        <Row style={{marginTop: "20px"}} >
          <Col style={{marginTop: "5px"}} span={(Setting.isMobile()) ? 19 : 2}>
            {Setting.getLabel(i18next.t("application:Enable LDAP signin"), i18next.t("application:Enable LDAP signin - Tooltip"))} :
          </Col>
          <Col span={1} >
            <Switch checked={this.state.application.enableLdapTab} onChange={checked => {
              this.updateApplicationField("enableLdapTab", checked);
            }} />
          </Col>
        </Row>
        <Row style={{marginTop: "20px"}} >
