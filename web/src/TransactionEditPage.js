...
class TransactionEditPage extends React.Component {
  ...
  getTransaction() {
    if (this.state.mode === "add" && this.props.location.transaction) {
      this.setState({
        transaction: this.props.location.transaction,
      });
      return;
    }
  ...
  submitTransactionEdit(exitAfterSave) {
    ...
    const apiCall = isAdd
      ? TransactionBackend.addTransaction(transaction)
    ...
    apiCall
      .then((res) => {
        if (res.status === "ok") {
          ...
          if (isAdd) {
            // the transaction name (ID) is generated by the server
            this.updateTransactionField("name", res.data);
          }
          this.setState({
          ...
          if (exitAfterSave) {
            ...
            this.props.history.push(`/transactions/${this.state.transaction.owner}/${this.state.transaction.name}`);
          }
        } else {
  ...
  deleteTransaction() {
    if (this.state.mode === "add") {
      // nothing has been persisted yet in the "add" mode
      this.props.history.push("/transactions");
      return;
    }
