    ...
    ProductBackend.buyProduct(product.owner, product.name, provider.name, pricingName, planName, userName, this.state.paymentEnv, customPrice)
      .then((res) => {
        if (res.status === "ok") {
          ...
          OrderBackend.updateOrder(order.owner, order.name, {
            ...order,
            payment: payment.name,
          }).then((updateRes) => {
            if (updateRes.status !== "ok") {
              Setting.showMessage("error", `Failed to update order: ${updateRes.msg}`);
            }
          });
  ...
  renderProviderButton(provider) {
    return (
      <span key={provider.name} style={{width: "200px", marginRight: "20px", marginBottom: "10px"}}>
        <span style={{width: "200px", cursor: "pointer"}} onClick={() => this.payOrder(provider)}>
    ...
    return (
      <div className="login-content">
        <Spin spinning={this.state.isProcessingPayment} size="large" tip={i18next.t("product:Processing payment...")} style={{paddingTop: "10%"}} >
          <Descriptions title={<span style={Setting.isMobile() ? {fontSize: 20} : {fontSize: 28}}>{i18next.t("order:Pay Order")}</span>} bordered>
            <Descriptions.Item label={i18next.t("order:Order ID")} span={3}>
              <span style={{fontSize: 20}}>
                {order.name}
              </span>
            </Descriptions.Item>
            <Descriptions.Item label={i18next.t("order:Order Status")} span={3}>
            ...
            <Descriptions.Item label={i18next.t("order:Select Payment Method")} span={3}>
