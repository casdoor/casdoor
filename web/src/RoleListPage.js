...
class RoleListPage extends BaseListPage {
  ...
  renderTable(roles) {
    ...
    return (
      <div>
        <Table scroll={{x: "max-content", y: 619}} columns={columns} dataSource={roles} rowKey={(record) => `${record.owner}/${record.name}`} size="middle" bordered pagination={paginationProps}
          title={() => (
            <div>
