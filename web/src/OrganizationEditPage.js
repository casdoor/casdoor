  ...
  renderOrganization() {
    return (
      <Card size="small" title={
        ...
        <Row style={{marginTop: "20px"}} >
          <Col style={{marginTop: "5px"}} span={(Setting.isMobile()) ? 22 : 2}>
            {Setting.getLabel(i18next.t("organization:Password pepper"), i18next.t("organization:Password pepper - Tooltip"))} :
          </Col>
          <Col span={22} >
            <Input value={this.state.organization.passwordPepper} onChange={e => {
              this.updateOrganizationField("passwordPepper", e.target.value);
            }} />
          </Col>
        </Row>
        <Row style={{marginTop: "20px"}} >
          <Col style={{marginTop: "5px"}} span={2}>
            {Setting.getLabel(i18next.t("organization:Password pepper mode"), i18next.t("organization:Password pepper mode - Tooltip"))} :
          </Col>
          <Col span={22}>
            <Select
              virtual={false}
              style={{width: "100%"}}
              value={this.state.organization.passwordPepperMode}
              onChange={value => this.updateOrganizationField("passwordPepperMode", value)}
              options={[
                {value: "", label: "None"},
                {value: "prefix", label: "Prefix (pepper + password)"},
                {value: "suffix", label: "Suffix (password + pepper)"},
              ]}
            />
          </Col>
        </Row>
