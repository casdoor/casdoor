...
func (c *ApiController) HandleLoggedIn(application *object.Application, user *object.User, form *RequestForm) (resp *Response) {
	...
	if !allowed {
		c.ResponseError(c.T("Unauthorized operation"))
		return
	}
	...
	if form.Type == ResponseTypeLogin {
		...
		if challengeMethod != "S256" && challengeMethod != "null" && challengeMethod != "" {
			c.ResponseError(c.T("Challenge method should be S256"))
			return
		}
	...
	if form.Username != "" {
		if form.Type == ResponseTypeLogin {
			if c.GetSessionUsername() != "" {
				c.ResponseError(c.T("Please sign out first before signing in"), c.GetSessionUsername())
				return
			}
		}
		...
		if form.Password == "" {
			...
			if strings.Contains(form.Username, "@") {
				...
				if len(form.PhonePrefix) == 0 {
					responseText := fmt.Sprintf(c.T("%s No phone prefix"), verificationCodeType)
			...
			if user == nil {
				c.ResponseError(fmt.Sprintf(c.T("The user: %s/%s doesn't exist"), form.Organization, form.Username))
				return
			}
		} else {
		...
		if msg != "" {
			...
			application := object.GetApplication(fmt.Sprintf("admin/%s", form.Application))
			if application == nil {
				c.ResponseError(fmt.Sprintf(c.T("The application: %s does not exist"), form.Application))
				return
			}
		...
		if !providerItem.IsProviderVisible() {
			c.ResponseError(fmt.Sprintf(c.T("The provider: %s is not enabled for the application"), provider.Name))
			return
		}
		...
		if provider.Category == "SAML" {
			...
			if idProvider == nil {
				c.ResponseError(fmt.Sprintf(c.T("The provider type: %s is not supported"), provider.Type))
				return
			}
			...
			if form.State != conf.GetConfigString("authState") && form.State != application.Name {
				c.ResponseError(fmt.Sprintf(c.T("State expected: %s, but got: %s"), conf.GetConfigString("authState"), form.State))
				return
			}
			...
			if !token.Valid() {
				c.ResponseError(c.T("Invalid token"))
				return
			}

			userInfo, err = idProvider.GetUserInfo(token)
			if err != nil {
				c.ResponseError(fmt.Sprintf(c.T("Failed to login in: %s"), err.Error()))
				return
			}
		}
		...
		if form.Method == "signup" {
			...
			if user != nil && !user.IsDeleted {
				...
				if user.IsForbidden {
					c.ResponseError(c.T("The user is forbidden to sign in, please contact the administrator"))
				}
				...
				if !application.EnableSignUp {
					c.ResponseError(fmt.Sprintf(c.T("The account for provider: %s and username: %s (%s) does not exist and is not allowed to sign up as new account, please contact your IT support"), provider.Type, userInfo.Username, userInfo.DisplayName))
					return
				}

				if !providerItem.CanSignUp {
					c.ResponseError(fmt.Sprintf(c.T("The account for provider: %s and username: %s (%s) does not exist and is not allowed to sign up as new account via %%s, please use another way to sign up"), provider.Type, userInfo.Username, userInfo.DisplayName, provider.Type))
					return
				}
				...
				if err != nil {
					c.ResponseError(fmt.Errorf(c.T("Get init score failed, error: %%w"), err).Error())
					return
				}
				...
				if !affected {
					c.ResponseError(fmt.Sprintf(c.T("Failed to create user, user information is invalid: %s"), util.StructToJson(user)))
					return
				}
			...
			if userId == "" {
				c.ResponseError(c.T("The account does not exist"), userInfo)
				return
			}

			oldUser := object.GetUserByField(application.Organization, provider.Type, userInfo.Id)
			if oldUser != nil {
				c.ResponseError(fmt.Sprintf(c.T("The account for provider: %s and username: %s (%s) is already linked to another account: %s (%s)"), provider.Type, userInfo.Username, userInfo.DisplayName, oldUser.Name, oldUser.DisplayName))
				return
			}
		...
		if c.GetSessionUsername() != "" {
			...
			c.ResponseError(fmt.Sprintf(c.T("Unknown authentication type (not password or provider), form = %s"), util.StructToJson(form)))
			return
		}
	}
