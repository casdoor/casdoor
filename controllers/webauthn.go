...
func (c *ApiController) WebAuthnSignupFinish() {
	...
	if !ok {
		c.ResponseError(c.T("Please call WebAuthnSigninBegin first"))
		return
	}
...
func (c *ApiController) WebAuthnSigninBegin() {
	...
	if user == nil {
		c.ResponseError(fmt.Sprintf(c.T("The user: %s/%s doesn't exist"), userOwner, userName))
		return
	}
	if len(user.WebauthnCredentials) == 0 {
		c.ResponseError(c.T("Found no credentials for this user"))
		return
	}
