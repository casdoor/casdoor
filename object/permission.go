...
func ContainsAsterisk(userId string, users []string) bool {
	...
	userSplit := strings.Split(userId, "/")
	var permissionUserSplit []string
	for _, user := range users {
		permissionUserSplit = strings.Split(user, "/")
		if permissionUserSplit[1] == "*" {
			containsAsterisk = true
			break
		}
	}
	if userSplit[0] == permissionUserSplit[0] && containsAsterisk {
		return true
	}
	return false
}
