...
func InitFromFile() {
	...
	if initDataFile == "" {
		logs.Info("InitFromFile: initDataFile config is not set, skipping initialization from file")
		return
	}

	logs.Info(fmt.Sprintf("InitFromFile: checking for init data file at path: %s", initDataFile))

	initDataNewOnly = conf.GetConfigBool("initDataNewOnly")
	if initDataNewOnly {
		logs.Info("InitFromFile: initDataNewOnly is enabled, existing data will not be replaced")
	}

	initData, err := readInitDataFromFile(initDataFile)
	if err != nil {
		logs.Error(fmt.Sprintf("InitFromFile: failed to read init data file: %s", err.Error()))
		panic(err)
	}

	if initData == nil {
		logs.Warning(fmt.Sprintf("InitFromFile: file not found at path: %s", initDataFile))
		return
	}
	...
	logs.Info("InitFromFile: initialization from file completed successfully")
}
