...
func getNotificationClient(provider *Provider) (notify.Notifier, error) {
	...
	client, err := notification.GetNotificationProvider(provider.Type, provider.ClientId, provider.ClientSecret, provider.AppId, provider.Receiver, provider.Method, provider.Title)
	if err != nil {
		return nil, err
	}
