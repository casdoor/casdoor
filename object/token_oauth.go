...
func GetOAuthCode(userId string, clientId string, provider string, signinMethod string, responseType string, redirectUri string, scope string, state string, nonce string, challenge string, host string, lang string) (*Code, error) {
	...
	accessToken, idToken, refreshToken, tokenName, err := generateJwtToken(application, user, provider, signinMethod, nonce, scope, host)
	if err != nil {
		return nil, err
	}
...
func RefreshToken(grantType string, refreshToken string, scope string, clientId string, clientSecret string, host string) (interface{}, error) {
	...
	newAccessToken, newIdToken, newRefreshToken, tokenName, err := generateJwtToken(application, user, "", "", "", scope, host)
	if err != nil {
		return &TokenError{
			Error:            EndpointError,
...
func GetPasswordToken(application *Application, username string, password string, scope string, host string) (*Token, *TokenError, error) {
	...
	accessToken, idToken, refreshToken, tokenName, err := generateJwtToken(application, user, "", "", "", scope, host)
	if err != nil {
		return nil, &TokenError{
			Error:            EndpointError,
			ErrorDescription: fmt.Sprintf("generate jwt token error: %s", err.Error()),
...
func GetClientCredentialsToken(application *Application, clientSecret string, scope string, host string) (*Token, *TokenError, error) {
	...
	token := &Token{
		...
		Code:         util.GenerateClientId(),
		AccessToken:  accessToken,
		IdToken:      "", // No id_token for client credentials flow
		ExpiresIn:    application.ExpireInHours * hourSeconds,
		Scope:        scope,
		TokenType:    "Bearer",
		CodeIsUsed:   true,
...
func GetTokenByUser(application *Application, user *User, scope string, nonce string, host string) (*Token, error) {
	...
	accessToken, idToken, refreshToken, tokenName, err := generateJwtToken(application, user, "", "", nonce, scope, host)
	if err != nil {
		return nil, err
	}
...
func GetWechatMiniProgramToken(application *Application, code string, host string, username string, avatar string, lang string) (*Token, *TokenError, error) {
	...
	accessToken, _, refreshToken, tokenName, err := generateJwtToken(application, user, "", "", "", "", host)
	if err != nil {
		return nil, &TokenError{
			Error:            EndpointError,
			ErrorDescription: fmt.Sprintf("generate jwt token error: %s", err.Error()),
	...
	token := &Token{
		...
		AccessToken:  accessToken,
		IdToken:      "", // WeChat miniprogram doesn't use id_token
		RefreshToken: refreshToken,
		ExpiresIn:    application.ExpireInHours * hourSeconds,
		Scope:        "",
		TokenType:    "Bearer",
