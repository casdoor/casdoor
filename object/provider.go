...
func (p *Provider) getPaymentProvider() (pp.PaymentProvider, *Cert, error) {
	...
	pProvider := pp.GetPaymentProvider(p.Type, p.ClientId, p.ClientSecret, p.Host, cert.Certificat, cert.PrivateKey, cert.AuthorityPublicKey, cert.AuthorityRootPublicKey)
	if pProvider == nil {
