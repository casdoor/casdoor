...
func FromProviderToIdpInfo(ctx *context.Context, provider *Provider) *idp.ProviderInfo {
	...
	if provider.Type == "WeChat" {
		...
		// The cert should contain the Alipay application private key (the one used to sign OAuth requests)
		cert, err := GetCert(util.GetId(provider.Owner, provider.Cert))
		if err != nil {
			logs.Warning("Failed to load cert %s for Alipay OAuth provider %s, falling back to ClientSecret: %v", provider.Cert, provider.Name, err)
		} else if cert == nil {
			logs.Warning("Cert %s does not exist for Alipay OAuth provider %s, falling back to ClientSecret", provider.Cert, provider.Name)
		} else if cert.PrivateKey == "" {
			logs.Warning("Cert %s for Alipay OAuth provider %s has empty private key, falling back to ClientSecret", provider.Cert, provider.Name)
		} else {
			// Use the private key from the cert as the client secret for OAuth signing
			// This should be the Alipay application private key (the key pair generated by Alipay)
			providerInfo.ClientSecret = cert.PrivateKey
			// For certificate mode, also set the certificate content for SN calculation
			// The cert.Certificate field should contain the application public key certificate (应用公钥证书)
			if cert.Certificate != "" {
		...
		if provider.Metadata != "" {
			...
			if err != nil {
				logs.Warning("Failed to load Alipay root cert %s: %v", provider.Metadata, err)
			} else if rootCert != nil && rootCert.PrivateKey != "" {
				providerInfo.AlipayRootCert = rootCert.PrivateKey
			}
		}
	}
