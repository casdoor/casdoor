...
func CheckUserSignup(application *Application, organization *Organization, authForm *form.AuthForm, lang string) string {
	...
	if application.IsSignupItemVisible("Username") {
		...
		if HasUserByField(organization.Name, "full_phone", authForm.FullPhone) {
			return i18n.Translate(lang, "check:Phone already exists")
		}
	}
...
func CheckUpdateUser(oldUser, user *User, lang string) string {
	...
	if oldUser.FullPhone != user.FullPhone {
		if HasUserByField(user.Owner, "full_phone", user.FullPhone) {
			return i18n.Translate(lang, "check:Phone already exists")
		}
	}
